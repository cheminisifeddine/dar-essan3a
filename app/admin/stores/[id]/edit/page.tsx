...
export default function EditStorePage() {
      ...
      .then((data) => {
        if (data.success && data.store) {
          setForm({
            ...
            text_color: data.store.text_color || "#241F18",
            primary_color: data.store.primary_color || "#1E3A2A",
            secondary_color: data.store.secondary_color || "#C19A3D",
            bg_color: data.store.bg_color || "#F7F2E9",
            hero_title: data.store.hero_title || "",
            hero_subtitle: data.store.hero_subtitle || "",
            announcement_bar: data.store.announcement_bar || "",
            description: data.store.description || "",
            whatsapp_number: data.store.whatsapp_number || "213558522110",
            pixel_id: data.store.pixel_id || "1459121952706477",
            home_delivery_fee: String(data.store.home_delivery_fee || 500),
            stopdesk_fee: String(data.store.stopdesk_fee || 350),
            free_shipping_threshold: String(data.store.free_shipping_threshold || 6000),
            active: !!data.store.active,
          });
        } else {
          setError(data.error || "تعذر تحميل بيانات المتجر");
        }
        setLoading(false);
      })
      .catch(() => {
  ...
  function handleSubdomainChange(val: string) {
    if (id === "main") return; // main subdomain cannot be edited
    const clean = val.toLowerCase().replace(/[^a-z0-9-]/g, "");
    setForm((f: any) => ({ ...f, subdomain: clean }));
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.success) {
        setSuccess("تم حفظ تعديلات المتجر بنجاح!");
        setTimeout(() => {
  ...
  if (loading) return <p className="text-muted">جارٍ تحميل بيانات المتجر…</p>;
  if (!form) return <p className="text-terracotta">المتجر غير موجود.</p>;
  ...
  return (
    <div className="max-w-4xl mx-auto pb-12">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="font-amiri text-3xl text-deepgreen">تعديل المتجر: {form.name}</h2>
          <p className="text-sm text-muted">
            تعديل الهوية البصرية، ألوان النصوص، والنطاق الفرعي تحت {MAIN_DOMAIN}
          </p>
        </div>
      ...
      {success && (
        <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl mb-6 font-bold">
          {success}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="flex items-center" dir="ltr">
                <input
                  ...
                  disabled={isMain}
                  className="w-full border border-gold/30 rounded-l-xl px-4 py-2.5 bg-white font-mono text-sm disabled:bg-gray-100"
                  required
                />
              ...
              {isMain ? (
                <p className="text-xs text-muted mt-1">هذا هو المتجر الرئيسي للموقع.</p>
              ) : (
        ...
        <div className="bg-ivory rounded-2xl p-6 shadow-soft border border-gold/15 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-2">
            ...
            <div className="bg-white p-3.5 rounded-xl border border-gold/20 shadow-sm">
              ...
              <p className="text-[11px] text-muted mt-1.5">لون العناوين، الأوصاف، والنصوص</p>
            </div>
            ...
            <div className="bg-white p-3.5 rounded-xl border border-gold/20 shadow-sm">
              ...
              <p className="text-[11px] text-muted mt-1.5">الهيدر، الأزرار، والفوتر</p>
            </div>
            ...
            <div className="bg-white p-3.5 rounded-xl border border-gold/20 shadow-sm">
              ...
              <p className="text-[11px] text-muted mt-1.5">شارات التخفيض ونقاط الميزات</p>
            </div>
            ...
            <div className="bg-white p-3.5 rounded-xl border border-gold/20 shadow-sm">
              ...
              <p className="text-[11px] text-muted mt-1.5">خلفية صفحات المتجر</p>
            </div>
          </div>
        ...
        <div className="bg-ivory rounded-2xl p-6 shadow-soft border border-gold/15 space-y-4">
          ...
          <div className="flex items-center gap-2 pt-2">
            ...
            />
            <label htmlFor="active" className="text-sm font-medium">المتجر نشط ومتاح للزبائن</label>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <button
            type="submit"
            disabled={saving}
            className="flex-1 bg-gold text-deepgreen font-bold text-lg py-4 rounded-xl hover:bg-gold/90 transition-colors shadow-soft disabled:opacity-70"
          >
            {saving ? "جارٍ الحفظ…" : "حفظ التعديلات"}
          </button>
          <Link
            href={`/?store=${encodeURIComponent(form.subdomain)}`}
            target="_blank"
            className="border border-gold/40 text-deepgreen font-bold text-center px-6 py-4 rounded-xl hover:bg-gold/10 transition-colors"
          >
            معاينة المتجر ↗
          </Link>
        </div>
